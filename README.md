<!--
    ✨ 欢迎来到我的 GitHub 主页！
    16 岁高中生 · Android 系统级定制爱好者 · Kotlin 忠实拥护者
    热爱干净、模糊、沉浸式的 UI，讨厌默认。
-->

<div align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=24&duration=3000&pause=500&color=6E56CF&center=true&vCenter=true&width=600&lines=Android+UI+Tinkerer;Kotlin+Enthusiast;Xposed+Module+Developer;Make+ColorOS+Great+Again" alt="Typing SVG" />

<br>
  <a href="https://github.com/JiaGuZhuangZhi/MCGA">
    <img src="https://img.shields.io/badge/MCGA-LSPosed%20Module-FF6F61?style=for-the-badge&logo=android&logoColor=white" />
  </a>
  <a href="https://github.com/JiaGuZhuangZhi/Gustate-AppBar">
    <img src="https://img.shields.io/badge/Gustate%20AppBar-Blur%20Header%20for%20Android-43B581?style=for-the-badge&logo=jetpackcompose&logoColor=white" />
  </a>
  <a href="https://github.com/Uotan-Dev/UOTAN-for-Android">
    <img src="https://img.shields.io/badge/UOTAN%20App-Forum%20Client-5865F2?style=for-the-badge&logo=github&logoColor=white" />
  </a>
  <br>
  🔧 正在用 Kotlin + Xposed 打造更美的 ColorOS  
  🎨 偏爱声明式 UI、背景模糊、沉浸体验  
  💡 代码即艺术，简洁即优雅
</div>

### 📦 我的开源项目

- **[MCGA](https://github.com/JiaGuZhuangZhi/MCGA)**  
  > 为 ColorOS 用户打造的轻量 LSPosed 模块，支持 Dock 模糊、隐藏应用名、AOD 全景沉浸等个性化定制。  
  *技术栈：Kotlin · Xposed · Jetpack Compose · haze*

- **[Gustate-AppBar](https://github.com/JiaGuZhuangZhi/Gustate-AppBar)**  
  > 一个支持背景模糊、系统栏适配、动态透明度的 Android 自定义 AppBar，开箱即用。  
  *技术栈：Kotlin · BlurView · ViewCompat · XML 属性扩展*

- **参与 [UOTAN for Android](https://github.com/Uotan-Dev/UOTAN-for-Android)**  
  > 柚坛社区官方安卓客户端，共建开源论坛生态。

### 📊 GitHub 统计

<div align="center">
  <img height="180em" src="https://github-readme-stats.vercel.app/api?username=JiaGuZhuangZhi&show_icons=true&theme=radical&count_private=true&include_all_commits=true" />
  <img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=JiaGuZhuangZhi&layout=compact&theme=radical&langs_count=6" />
</div>

### 💻 最常用的语言

<div align="center">
  <img src="https://skillicons.dev/icons?i=kotlin,java,gradle,git,github" />
</div>

### 🎯 开发理念

- 坚信 **UI 应服务于体验**，而非干扰  
- 喜欢用 **声明式方式** 构建界面（讨厌 XAML 😅）  
- 目标：让 Android 系统更自由、更美、更“我”

---

> 📌 *本页由高中生独立维护，代码开源，无商业目的。欢迎 Star、Issue 或 PR！*  
> ⏳ 最后更新：2025 年 11 月
